...
export default function ProgramsAdmin() {
  ...
  const save = async () => {
    ...
    if (!form.name.trim() || !form.level.trim() || !form.code.trim()) {
    ...
    if (!editItem) {
      const exists = items.some(
        (p) =>
          ...
          p.code.toLowerCase() === form.code.toLowerCase()
      );

      if (exists) {
        setError("Ce programme existe déjà.");
        return;
      }
    }
    ...
    try {
      if (editItem) {
        await api.put(
          `/academic/programs/${editItem._id}`,
          form,
          {
            headers: { "x-college-id": "69130f6ec3818024a5c994c1" }
          }
    ...
    <div className="container-page py-8">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        <h2 className="text-2xl font-semibold mb-4 text-white">
          {editItem ? "Modifier le programme" : "Ajouter un programme"}
        </h2>
